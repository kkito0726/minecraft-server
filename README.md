...
| `MC_VERSION` | Minecraft のバージョン。**クライアントと必ず一致させる**。管理コンソールでワールドを切り替えると、そのワールドの版に書き換わる |
...
|---|---|
| `/` | 状態表示、起動・停止・再起動 |
| `/worlds` | 一覧・新規作成（版・モード・難易度を選ぶ。ハードコアはモードの選択肢で、選ぶと難易度はハードに固定）・複製・改名・削除・**切り替え**（サーバーの版とハードコアは切り替え先のワールドに合わせる） |
| `/backups` | 取得・一覧・削除・保持ポリシー・**復元**、zip の取り込みと手元の PC への持ち出し |
| `/settings` | モード・難易度・MOTD・最大人数・描画距離・シミュレーション距離（保存だけ / 今すぐ反映） |
| `/system` | ホスト（Pi 本体）の CPU・メモリ・ストレージの使用状況。開いている間だけ 5 秒ごとに更新 |
