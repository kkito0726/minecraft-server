...
### 画面から一往復して測ったこと（2026-09-10、フェーズ 17 の確認）

`data/world` を複製した `uitest17` に切り替えたうえで、ブラウザから取得と復元を通した。

| 確認したこと | 結果 |
|---|---|
| メモに日本語を入れたとき | 打っている最中に「このメモはファイル名には残りません」と出る。`ui check` に直すと「末尾に `ui-check` が付きます」に変わる |
| HOT 取得（25.2MB のワールド） | `backup-26.2-uitest17-20260910-214402-ui-check.zip` が 10.9MB。3 ステップが 1 秒台で完了 |
| 一覧に出るバージョン | `26.2`。ファイル名からではなくアーカイブ内の `level.dat` から読んだ値 |
| 同じワールドへ戻す復元の関門 | 承諾のチェックは**出ない**（`verdict=MATCH` かつ名前も同じ）。名前を `uitest1` まで打った時点では押せず、`uitest17` で押せるようになる |
| 復元の所要 | 7 ステップで 19 秒（停止 → 退避 → 展開 → 設定 → 起動） |
| 取得後に足したファイル | 復元後の `data/uitest17/` から消え、`data/uitest17.broken-20260910-214428/` に残っていた。ブラウザ経由でも退避が展開より先である |
| `world` に戻してから同じアーカイブを開いたとき | 名前の食い違いを検出して承諾のチェックが出る。名前だけでは押せず、承諾を足すと押せる |
| その状態で復元先を「稼働中のワールド名」に切り替えたとき | 打ち込んだ `uitest17` が消え、求められる名前が `world` に変わる |

最後の 1 行が、この画面で唯一気づきにくい落とし穴だった。切り替えても入力が残る作りにすると、
**正しい名前を打ったのにボタンが押せず、理由が画面のどこにも出ない**状態になる。
