...
**`MC_VERSION` を書き換えたら、切り替えを挟まずにそのまま起動する。** 管理コンソールの
ワールドの切り替えは、`MC_VERSION` を切り替え先のワールドが最後に開かれた版に合わせる。
書き換えた直後（まだ新しい版で一度も開いていない）に画面から切り替えると、書き換えが
黙って元の版に戻る。一度起動すれば `level.dat` が新しい版になり、以後の切り替えもそれに揃う。

版上げは**稼働中のワールドだけ**に効く。他のワールドはそれぞれの版のまま残り、
切り替えるとサーバーの版もそのワールドの版に戻る。全部を上げたい場合は、ワールドごとに
切り替えてからこの手順を繰り返す。
