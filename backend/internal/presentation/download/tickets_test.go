...
/*
上限に達していても、期限内の券は引き換えられること。

上限の強制は「券が増える側」の関心なので Issue だけが行う。Redeem が
一緒に行うと、飽和状態では引き換えのたびに必ず 1 枚が追い出される。
追い出されるのは期限がいちばん古いもの＝利用者が最初に押したリンクで、
押した本人には理由が分からないまま失敗する。

券は 1 枚ずつ時刻をずらして発行する。同じ時刻に揃えると期限も揃い、
「いちばん古い」が定まらないので、追い出しの対象が試行ごとに変わる。
*/
func TestRedeemAtCapKeepsOldestValidTicket(t *testing.T) {
	t.Parallel()

	now := time.Date(2026, 9, 16, 12, 0, 0, 0, time.UTC)
	tickets := newTestTickets(&now)

	first := issueToken(t, tickets, "backup-first.zip")
	for range maxTickets - 1 {
		now = now.Add(time.Second)
		issueToken(t, tickets, "backup.zip")
	}

	id, ok := tickets.Redeem(first)
	if !ok || id != "backup-first.zip" {
		t.Errorf("期限内の券が引き換えられない: %q %v", id, ok)
	}
}

// Range による転送の再開も同じ経路を通る。飽和状態でも続けて通ること。
func TestRedeemAtCapIsRepeatable(t *testing.T) {
	t.Parallel()
